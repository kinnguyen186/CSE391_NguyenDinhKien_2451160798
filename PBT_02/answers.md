...
7. `type="radio"` → Ô tròn nhỏ, chỉ chọn 1 trong nhóm cùng name → Dùng cho chọn phương thức thanh toán.
8. `type="checkbox"` → Ô vuông nhỏ, click để tick/bỏ tick, dùng required để bắt buộc → Dùng cho đồng ý điều khoản.
9. `type="search"` → Ô nhập text có icon 🔍 kèm nút xóa nhanh, không tự validate → Dùng cho thanh tìm kiếm sản phẩm.
10. `type="file"` → Nút chọn tệp, giới hạn định dạng bằng accept → Dùng cho upload ảnh đánh giá sản phẩm.

_Câu A2_

- TH1: Form không submit vì `required` bắt buộc phải có giá trị. `value=""` = rỗng → vi phạm, trình duyệt sẽ chặn lại và hiển thị thông báo lỗi (tooltip) yêu cầu người dùng điền vào trường này.
- TH2: Form không submit vì `type="email"` tự validate định dạng phải có `@` và domain. `"abc"` không có `@` → sai định dạng, hiện tooltip yêu cầu người dùng nhập đúng định dạng.
- TH3: Form không submit vì `max="10"` giới hạn giá trị tối đa. `15 > 10` → vi phạm, hiện tooltip "Giá trị phải nhỏ hơn hoặc bằng 10".
- TH4: Form không submit vì `pattern` yêu cầu đúng 10 chữ số liên tiếp. `"abc123"` có chữ cái và chỉ 6 ký tự → không khớp regex, hiện tooltip yêu cầu nhập đúng định dạng.
...
<fieldset>
  ...
  <input type="radio" id="titan-black" name="color" value="black" checked />
  <label for="titan-black">Đen Titan</label>
  <input type="radio" id="titan-white" name="color" value="white" />
  <label for="titan-white">Trắng Titan</label>
  <input type="radio" id="titan-natural" name="color" value="natural" />
  <label for="titan-natural">Titan Tự Nhiên</label>
</fieldset>
```
